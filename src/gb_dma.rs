...
pub struct GbWriteDmaConfig<'d> {
    _dma_ch0: PeripheralRef<'d, AnyChannel>,
    _dma_ch1: PeripheralRef<'d, AnyChannel>,
    _dma_ch2: PeripheralRef<'d, AnyChannel>,
}

impl<'d> GbWriteDmaConfig<'d> {
    pub fn new(
        dma0: impl Peripheral<P = impl Channel> + 'd,
        dma1: impl Peripheral<P = impl Channel> + 'd,
        dma2: impl Peripheral<P = impl Channel> + 'd,
        write_base_addr_ptr: *mut *mut u8,
        read_target: &dyn DmaReadTarget<ReceivedWord = u32>,
    ) -> Self {
        into_ref!(dma0);
        into_ref!(dma1);
        into_ref!(dma2);
        ...
        // setup channel 1, which reads the data and writes it to the designated addr, do not trigger it, channel 2 will trigger it
        p1.trans_count().write(|w| {
            w.set_count(1);
        });
        p1.read_addr().write_value(read_target.rx_address_count().0);
        let mut dma1_cfg = pac::dma::regs::CtrlTrig(0);
        dma1_cfg.set_incr_read(false);
        dma1_cfg.set_incr_write(false);
        dma1_cfg.set_treq_sel(pac::dma::vals::TreqSel::from(
            read_target
                .rx_treq()
                .unwrap_or(pac::dma::vals::TreqSel::PERMANENT as u8),
        ));
        ...
        /* setup channel 0, which reads from the RX FIFO on DREQ, and writes the addr to the write addr register of channel 1. Trigger the channel, so it will start on DREQ */
        p0.trans_count().write(|w| {
            w.set_count(1);
        });
        p0.read_addr().write_value(read_target.rx_address_count().0);
        p0.write_addr().write_value(p1.write_addr().as_ptr() as u32);
        p0.ctrl_trig().write(|w| {
            w.set_incr_read(false);
            w.set_incr_write(false);
            w.set_treq_sel(pac::dma::vals::TreqSel::from(
        ...
        /* setup channel 2, which reads the base addr and writes it to the write addr of channel 1, while triggering it. The write happens through the set-shadow register, to combine it with the addr written before from channel 0*/
        p2.trans_count().write(|w| {
            w.set_count(1);
        });
        p2.read_addr().write_value(write_base_addr_ptr as u32);
        p2.write_addr()
            .write_value((p1.al2_write_addr_trig().as_ptr() as u32) | REG_ALIAS_SET_BITS);
        p2.ctrl_trig().write(|w| {
            w.set_incr_read(false);
            w.set_incr_write(false);
            w.set_treq_sel(pac::dma::vals::TreqSel::PERMANENT);
